...
main(void)
{
   ...
   3:	83 e4 f0             	and    $0xfffffff0,%esp
    frrTest();
   6:	e8 45 00 00 00       	call   50 <frrTest>
    exit();
...
print_sth(int number)
{
  ...
  13:	56                   	push   %esi
  14:	53                   	push   %ebx
  15:	bb e8 03 00 00       	mov    $0x3e8,%ebx
  ...
  24:	c7 44 24 04 08 08 00 	movl   $0x808,0x4(%esp)
  2b:	00 
  2c:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
  33:	e8 68 04 00 00       	call   4a0 <printf>
...
frrTest(void)
{
  ...
  6c:	83 fb 0a             	cmp    $0xa,%ebx
  6f:	75 ef                	jne    60 <frrTest+0x10>
  71:	30 db                	xor    %bl,%bl
  73:	90                   	nop
  ...
  a0:	c7 44 24 04 10 08 00 	movl   $0x810,0x4(%esp)
  a7:	00 
  ...
  c0:	83 c3 01             	add    $0x1,%ebx
        printf(1, "The waittime of %d child is %d\n", k, wTime[k]);
        printf(1, "The runtime of %d child is %d\n", k, rTime[k]);
  c3:	c7 44 24 04 30 08 00 	movl   $0x830,0x4(%esp)
  ca:	00 
  ...
  d6:	e8 c5 03 00 00       	call   4a0 <printf>
    ...
    int k;
    for (k = 0; k < CHILD_NUMBER; ++k) {
  db:	83 fb 0a             	cmp    $0xa,%ebx
  de:	75 b8                	jne    98 <frrTest+0x48>
        printf(1, "The waittime of %d child is %d\n", k, wTime[k]);
        printf(1, "The runtime of %d child is %d\n", k, rTime[k]);
    }

}
  e0:	83 c4 60             	add    $0x60,%esp
  e3:	5b                   	pop    %ebx
  e4:	5e                   	pop    %esi
  e5:	5d                   	pop    %ebp
  e6:	c3                   	ret    
    ...
    for (i = 0; i < CHILD_NUMBER; i++) {
        pid[i] = fork();
        if(pid[i] == 0){
  e7:	be e8 03 00 00       	mov    $0x3e8,%esi
...
print_sth(int number)
{
  ...
  f4:	c7 44 24 04 08 08 00 	movl   $0x808,0x4(%esp)
  fb:	00 
  fc:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
 103:	e8 98 03 00 00       	call   4a0 <printf>
...
print_sth(int number)
{
    int i;
    for (i = 0; i < N; ++i) {
 108:	83 ee 01             	sub    $0x1,%esi
 10b:	75 e3                	jne    f0 <frrTest+0xa0>
    int i;
    for (i = 0; i < CHILD_NUMBER; i++) {
        pid[i] = fork();
        if(pid[i] == 0){
            print_sth(i);
            exit();
 10d:	e8 30 02 00 00       	call   342 <exit>
...
printint(int fd, int xx, int base, int sgn)
{
 ...
 439:	0f b6 92 57 08 00 00 	movzbl 0x857(%edx),%edx
  }while((x /= base) != 0);
...
putc(int fd, char c)
{
 ...
 5cf:	8b 38                	mov    (%eax),%edi
        ...
        if(s == 0)
          s = "(null)";
 5d1:	b8 50 08 00 00       	mov    $0x850,%eax
...
free(void *ap)
{
 ...
 6ff:	03 4b fc             	add    -0x4(%ebx),%ecx
    ...
    p->s.ptr = bp;
  freep = p;
 702:	a3 20 0b 00 00       	mov    %eax,0xb20
    bp->s.size += p->s.ptr->s.size;
    bp->s.ptr = p->s.ptr->s.ptr;
  } else
    bp->s.ptr = p->s.ptr;
...
malloc(uint nbytes)
{
 ...
 729:	8b 45 08             	mov    0x8(%ebp),%eax
  if((prevp = freep) == 0){
 72c:	8b 1d 20 0b 00 00    	mov    0xb20,%ebx
malloc(uint nbytes)
{
  Header *p, *prevp;
  uint nunits;
...
morecore(uint nu)
{
 ...
 769:	89 c2                	mov    %eax,%edx
    ...
    if(p == freep)
 76b:	3b 15 20 0b 00 00    	cmp    0xb20,%edx
...
morecore(uint nu)
{
 ...
 79d:	e8 ee fe ff ff       	call   690 <free>
  return freep;
 7a2:	8b 15 20 0b 00 00    	mov    0xb20,%edx
      }
      freep = prevp;
      return (void*)(p + 1);
    }
 ...
 7dc:	eb e9                	jmp    7c7 <malloc+0xa7>
  ...
  if((prevp = freep) == 0){
    base.s.ptr = freep = prevp = &base;
 7de:	c7 05 20 0b 00 00 24 	movl   $0xb24,0xb20
 7e5:	0b 00 00 
    base.s.size = 0;
 7e8:	ba 24 0b 00 00       	mov    $0xb24,%edx
  Header *p, *prevp;
  uint nunits;
  ...
  if((prevp = freep) == 0){
    base.s.ptr = freep = prevp = &base;
 7ed:	c7 05 24 0b 00 00 24 	movl   $0xb24,0xb24
 7f4:	0b 00 00 
    base.s.size = 0;
