...
main(void)
{
   ...
   3:	83 e4 f0             	and    $0xfffffff0,%esp
    RRsanity();
   6:	e8 45 00 00 00       	call   50 <RRsanity>
    exit();
...
print_sth(int number)
{
  ...
  24:	83 c3 01             	add    $0x1,%ebx
        printf(1, "Child %d prints for the %d time.\n", number, i);
  27:	89 74 24 08          	mov    %esi,0x8(%esp)
  2b:	c7 44 24 04 08 08 00 	movl   $0x808,0x4(%esp)
  32:	00 
...
print_sth(int number)
{
    int i;
    for (i = 0; i < N; ++i) {
  3f:	81 fb e8 03 00 00    	cmp    $0x3e8,%ebx
  45:	75 d9                	jne    20 <print_sth+0x10>
        printf(1, "Child %d prints for the %d time.\n", number, i);
    }
...
RRsanity(void)
{
  ...
  59:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
    ...
    for (i = 0; i < CHILD_NUMBER; i++) {
        pid[i] = fork();
  60:	e8 d5 02 00 00       	call   33a <fork>
        if(pid[i] == 0){
  65:	85 c0                	test   %eax,%eax
  67:	0f 84 9d 00 00 00    	je     10a <RRsanity+0xba>
    int wTime[CHILD_NUMBER];
    int rTime[CHILD_NUMBER];
    int pid[CHILD_NUMBER];
  ...
  73:	75 eb                	jne    60 <RRsanity+0x10>
  75:	30 db                	xor    %bl,%bl
  77:	90                   	nop
  78:	8d 45 d0             	lea    -0x30(%ebp),%eax
  7b:	01 d8                	add    %ebx,%eax
        }
    }
  ...
  81:	8d 45 a8             	lea    -0x58(%ebp),%eax
  84:	01 d8                	add    %ebx,%eax
  86:	83 c3 04             	add    $0x4,%ebx
  89:	89 04 24             	mov    %eax,(%esp)
  8c:	e8 59 03 00 00       	call   3ea <wait2>
            exit();
        }
    }
    ...
    int j;
    for (j = 0; j < CHILD_NUMBER; ++j) {
  91:	83 fb 28             	cmp    $0x28,%ebx
  94:	75 e2                	jne    78 <RRsanity+0x28>
  96:	30 db                	xor    %bl,%bl
        wait2(&wTime[j], &rTime[j]);
    }
  ...
  98:	8b 44 9d a8          	mov    -0x58(%ebp,%ebx,4),%eax
  9c:	89 5c 24 08          	mov    %ebx,0x8(%esp)
  a0:	c7 44 24 04 2c 08 00 	movl   $0x82c,0x4(%esp)
  a7:	00 
  a8:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
  af:	89 44 24 0c          	mov    %eax,0xc(%esp)
  b3:	e8 e8 03 00 00       	call   4a0 <printf>
        printf(1, "The runtime of %d child is %d\n", k, rTime[k]);
  b8:	8b 44 9d d0          	mov    -0x30(%ebp,%ebx,4),%eax
  bc:	89 5c 24 08          	mov    %ebx,0x8(%esp)
  c0:	c7 44 24 04 4c 08 00 	movl   $0x84c,0x4(%esp)
  c7:	00 
  ...
  d8:	8b 44 9d a8          	mov    -0x58(%ebp,%ebx,4),%eax
  dc:	03 44 9d d0          	add    -0x30(%ebp,%ebx,4),%eax
  ...
  e4:	83 c3 01             	add    $0x1,%ebx
        ...
        printf(1, "The runtime of %d child is %d\n", k, rTime[k]);
        printf(1, "The turn around time of %d child is %d\n", k, (wTime[k] + rTime[k]));
  e7:	c7 44 24 04 6c 08 00 	movl   $0x86c,0x4(%esp)
  ee:	00 
  ef:	89 44 24 0c          	mov    %eax,0xc(%esp)
  f3:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
  fa:	e8 a1 03 00 00       	call   4a0 <printf>
    for (j = 0; j < CHILD_NUMBER; ++j) {
        wait2(&wTime[j], &rTime[j]);
    }
    ...
    int k;
    for (k = 0; k < CHILD_NUMBER; ++k) {
  ff:	83 fb 0a             	cmp    $0xa,%ebx
 102:	75 94                	jne    98 <RRsanity+0x48>
        printf(1, "The waittime of %d child is %d\n", k, wTime[k]);
        printf(1, "The runtime of %d child is %d\n", k, rTime[k]);
        printf(1, "The turn around time of %d child is %d\n", k, (wTime[k] + rTime[k]));
    }
 ...
 10d:	e8 fe fe ff ff       	call   10 <print_sth>
            exit();
 112:	e8 2b 02 00 00       	call   342 <exit>
...
printint(int fd, int xx, int base, int sgn)
{
 ...
 439:	0f b6 92 9b 08 00 00 	movzbl 0x89b(%edx),%edx
  }while((x /= base) != 0);
...
putc(int fd, char c)
{
 ...
 5cf:	8b 38                	mov    (%eax),%edi
        ...
        if(s == 0)
          s = "(null)";
 5d1:	b8 94 08 00 00       	mov    $0x894,%eax
...
free(void *ap)
{
 ...
 6d7:	89 10                	mov    %edx,(%eax)
  freep = p;
 6d9:	a3 60 0b 00 00       	mov    %eax,0xb60
}
 6de:	5b                   	pop    %ebx
 6df:	5e                   	pop    %esi
 6e0:	5f                   	pop    %edi
 ...
 6ff:	03 4b fc             	add    -0x4(%ebx),%ecx
    ...
    p->s.ptr = bp;
  freep = p;
 702:	a3 60 0b 00 00       	mov    %eax,0xb60
    bp->s.size += p->s.ptr->s.size;
    bp->s.ptr = p->s.ptr->s.ptr;
  } else
    bp->s.ptr = p->s.ptr;
...
malloc(uint nbytes)
{
 ...
 729:	8b 45 08             	mov    0x8(%ebp),%eax
  if((prevp = freep) == 0){
 72c:	8b 1d 60 0b 00 00    	mov    0xb60,%ebx
malloc(uint nbytes)
{
  Header *p, *prevp;
  uint nunits;
...
morecore(uint nu)
{
 ...
 769:	89 c2                	mov    %eax,%edx
    ...
    if(p == freep)
 76b:	3b 15 60 0b 00 00    	cmp    0xb60,%edx
...
morecore(uint nu)
{
 ...
 79d:	e8 ee fe ff ff       	call   690 <free>
  return freep;
 7a2:	8b 15 60 0b 00 00    	mov    0xb60,%edx
      }
      freep = prevp;
      return (void*)(p + 1);
    }
 ...
 7dc:	eb e9                	jmp    7c7 <malloc+0xa7>
  ...
  if((prevp = freep) == 0){
    base.s.ptr = freep = prevp = &base;
 7de:	c7 05 60 0b 00 00 64 	movl   $0xb64,0xb60
 7e5:	0b 00 00 
    base.s.size = 0;
 7e8:	ba 64 0b 00 00       	mov    $0xb64,%edx
  Header *p, *prevp;
  uint nunits;
  ...
  if((prevp = freep) == 0){
    base.s.ptr = freep = prevp = &base;
 7ed:	c7 05 64 0b 00 00 64 	movl   $0xb64,0xb64
 7f4:	0b 00 00 
    base.s.size = 0;
 7f7:	c7 05 68 0b 00 00 00 	movl   $0x0,0xb68
 7fe:	00 00 00 
