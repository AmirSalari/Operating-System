...
#define FIFO_ROUND_ROBIN_Policy 2
#define GRT_Policy 3
#define MLQ_Policy 4
...
nice(){
    if (proc){
        if (proc->prior == prior_G){
            proc->prior = prior_FRR;
            return 0;
        } else if (proc->prior == prior_FRR) {
            proc->prior = prior_RR;
            return 0;
        } else if (proc->prior == prior_RR){
            return -1;
        }
    }
    return -1;
...
run_guaranteed(void){
    ...
    int to_exec = -1;
    for(nnp = ptable.proc; nnp < &ptable.proc[NPROC]; nnp++) {
        double grt = (double) (nnp->rtime / (ticks - nnp->ctime + 1)); // +1 for avoid to zero
...
run_mlq(void){
    ...
    for(nnp = ptable.proc; nnp < &ptable.proc[NPROC]; nnp++) {
        ...
        if (nnp->state == RUNNABLE && grt <= mingrt && nnp->prior == prior_G){
            mingrt = grt;
            to_exec = nnp->pid;
        }
    }
    if (to_exec == -1){
        for(nnp = ptable.proc; nnp < &ptable.proc[NPROC]; nnp++){
            if(nnp->state == RUNNABLE && nnp->prior == prior_FRR){
    ...
    if (to_exec == -1){
        for(nnp = ptable.proc; nnp < &ptable.proc[NPROC]; nnp++) {
            if (nnp->state == RUNNABLE && nnp->prior == prior_RR){
                to_exec = nnp->pid;
            }
        }
    }
    ...
    for(;;){
        ...
        switch (SCHED_TYPE) {
           case ROUND_ROBIN_Policy:
               finalized_pid = run_rr();
               break;
           case FIFO_ROUND_ROBIN_Policy:
               finalized_pid = run_fifo();
               break;
           case GRT_Policy:
               finalized_pid = run_guaranteed();
               break;
           case MLQ_Policy:
               finalized_pid = run_mlq();
               break;
        }
        for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
